...
namespace TextSpeecher
{
    ...
    public partial class MainForm : Form
    {
            ...
            if (listBoxVoices != null)
            {
                ...
                if (listBoxVoices.Items.Count > 0)
                {
                    ...
                    listBoxVoices.SelectedIndexChanged += (s, e) =>
                    {
                        ...
                        string selectedVoice = listBoxVoices.SelectedItem != null ? listBoxVoices.SelectedItem.ToString() ?? string.Empty : string.Empty;
                        // Set the synthesizer to use the selected voice
                        if (synthesizer != null && !string.IsNullOrEmpty(value: selectedVoice))
                        {
        ...
        // Ersetze alle direkten Verwendungen von 'synthesizer' durch eine Null-Prüfung.
        // Beispiel: synthesizer.SetOutputToDefaultAudioDevice();
        // wird zu: synthesizer?.SetOutputToDefaultAudioDevice();
        ...
        // Ersetze alle direkten Verwendungen von 'synthesizer' durch eine Null-Prüfung.
        // Beispiel: synthesizer.Volume -> synthesizer?.Volume ?? 0

        // Beispielhafte Anpassung für die Methode TrackBarVolume_Enter:
        private void TrackBarVolume_Enter(object sender, EventArgs e)
        {
            ...
            SetStatusBar(text: "Adjust the volume of the speech.", additionalInfo: $"Current Volume: {synthesizer?.Volume ?? 0}");
        }

        // Beispielhafte Anpassung für die Methode LabelVolume_Enter:
        private void LabelVolume_Enter(object sender, EventArgs e)
        {
            SetStatusBar(text: "Adjust the volume of the speech.", additionalInfo: $"Current Volume: {synthesizer?.Volume ?? 0}");
        }

        // Beispielhafte Anpassung für die Methode TrackBarSpeechRate_Enter:
        private void TrackBarSpeechRate_Enter(object sender, EventArgs e)
        {
            SetStatusBar(text: "Adjust the speech rate.", additionalInfo: $"Current Speech Rate: {synthesizer?.Rate ?? 0}");
        }

        // Beispielhafte Anpassung für die Methode LabelSpeechRate_Enter:
        ...
        // Ersetze in TrackBarVolume_Scroll die direkte Verwendung von 'synthesizer' durch eine Null-Prüfung.
        ...
        // Ersetze in TrackBarSpeechRate_Scroll die direkte Verwendung von 'synthesizer' durch eine Null-Prüfung.
        ...
        {
            // Prüfe, ob der Synthesizer initialisiert ist
            if (synthesizer == null)
            {
                _ = MessageBox.Show(text: "The SpeechSynthesizer instance is not initialized.", caption: "Error", buttons: MessageBoxButtons.OK, icon: MessageBoxIcon.Error);
                return;
            }
            // Schalte zwischen Pause und Fortsetzen basierend auf dem aktuellen Status
        ...
        // Ersetze alle direkten Verwendungen von 'synthesizer' durch eine Null-Prüfung in den betroffenen Methoden
        ...
        // Ersetze in der Methode ButtonShowVoiceInfo_Click die Zeile
        // VoiceInfo? selectedVoiceInfo = synthesizer.Voice;
        // durch eine Null-Prüfung für synthesizer:

        private void ButtonShowVoiceInfo_Click(object sender, EventArgs e)
        {
            // Füge eine Null-Prüfung für synthesizer hinzu
            if (synthesizer == null)
            {
                _ = MessageBox.Show(text: "The SpeechSynthesizer instance is not initialized.", caption: "Error", buttons: MessageBoxButtons.OK, icon: MessageBoxIcon.Error);
                return;
        ...
        private void MainForm_DragDrop(object sender, DragEventArgs e)
        {
            try
            {
                // Sicherstellen, dass e.Data nicht null ist, bevor GetData aufgerufen wird
                if (e.Data != null && e.Data.GetData(format: DataFormats.FileDrop) is string[] files && files.Length > 0)
                {
        ...
        {
            // Prüfe, ob e.Data nicht null ist, bevor GetDataPresent aufgerufen wird
            e.Effect = e.Data != null && e.Data.GetDataPresent(format: DataFormats.FileDrop) ? DragDropEffects.Copy : DragDropEffects.None;
        }
        ...
        private void TextBox_MouseDown(object sender, MouseEventArgs e)
        {
            if (!string.IsNullOrEmpty(value: textBox.Text))
            {
                // Starts the drag & drop process
                textBox.DoDragDrop(data: textBox.Text, allowedEffects: DragDropEffects.Copy);
            }
        }
    }
